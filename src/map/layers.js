...
export function addDataLayers() {
  ...
  const zoomInterpolation = [
    ...
    ['zoom'],
    12,
    ['/', ['coalesce', ['get', 'widthM'], 7], 1.0],
    16,
    ['*', ['coalesce', ['get', 'widthM'], 7], 2.4],
    20,
    ['*', ['coalesce', ['get', 'widthM'], 7], 8.0],
  ];
  ...
  state.map.addLayer({
    ...
    paint: {
      ...
      'line-width': [
        ...
        ['zoom'],
        12,
        ['/', ['coalesce', ['get', 'widthM'], 2], 1.0],
        16,
        ['*', ['coalesce', ['get', 'widthM'], 2], 2.4],
        20,
        ['*', ['coalesce', ['get', 'widthM'], 2], 8.0],
      ],
    },
  });
  ...
  const zoneFilter = [
    'match',
    ['get', 'type'],
    ['zone', 'park', 'terrain', 'water', 'radius'],
    true,
    false,
  ];
  state.map.addLayer({
  ...
  const bldFilter = [
    'match',
    ['get', 'type'],
    ['house', 'building', 'custom_building'],
    true,
    false,
  ];
  state.map.addLayer({
  ...
  // Draw preview
  state.map.addSource('draw-preview', {
    type: 'geojson',
  ...
  // Edit Handles Layer
  state.map.addSource('edit-handles', {
    type: 'geojson',
    data: { type: 'FeatureCollection', features: [] },
  });
  state.map.addLayer({
    id: 'layer-edit-handles',
    type: 'circle',
    source: 'edit-handles',
    paint: {
  ...
  state.map.addSource('edge-highlight', {
    type: 'geojson',
    data: { type: 'FeatureCollection', features: [] },
  });
  state.map.addLayer({
    id: 'layer-edge-highlight',
    type: 'line',
...
export function setupLayerInteractivity() {
  ...
  state.map.on('mouseleave', 'layer-edit-handles', () => {
  ...
  [
    ...
    state.map.on('mouseenter', lid, () => {
      if (['select', 'delete', 'move'].includes(state.tool))
        state.map.getCanvas().style.cursor =
          state.tool === 'delete' ? 'not-allowed' : state.tool === 'move' ? 'grab' : 'pointer';
    });
    state.map.on('mouseleave', lid, () => {
      state.map.getCanvas().style.cursor = '';
    });
  });
}
