...
export function finishLine() {
  ...
  const feat = {
    ...
    properties: {
      ...
      curved: document.getElementById('lineCurved')?.checked || false,
      lanes: lanes,
      widthM: widthM,
    },
    geometry: {
      type: 'LineString',
      coordinates:
        document.getElementById('lineCurved')?.checked && pts.length > 2 ? catmullRom(pts) : pts,
    },
  };
  pushHistory();
  addFeatures(feat);
...
export function finishRadius(lng, lat) {
  ...
  addFeatures({
    ...
    properties: {
      ...
      raw_pts: [...pts],
      radius_m: r_m,
      area_m2: Math.round(Math.PI * r_m * r_m),
    },
    geometry: { type: 'Polygon', coordinates: [pts] },
  });
  pushHistory();
...
export function updateDrawPreview() {
  ...
  if (pts.length >= 2) {
    ...
    const curved =
      (['road', 'path', 'sidewalk', 'railway'].includes(state.tool) &&
        document.getElementById('lineCurved')?.checked) ||
