...
export function handleMouseMove(e) {
  ...
  if (state.draggingFeatureId && state.tool === 'move') {
    ...
    const toMove = state.selectedIds.includes(state.draggingFeatureId)
      ? state.selectedIds
      : [state.draggingFeatureId];
...
export function handleMapClick(e) {
  ...
  if (['house', 'building'].includes(state.tool)) {
    placeBuilding(state.tool, lng, lat);
    return;
  } else if (state.tool === 'tree') {
    const treeType = document.getElementById('treeType')?.value;
    finishTree(lng, lat, treeType);
    return;
    ...
    finishFurniture(lng, lat, furnitureType, furnitureRot);
    return;
  } else if (state.tool === 'radius') {
    finishRadius(lng, lat);
    return;
  } else {
    ...
    if (state.drawPoints.length > 0) {
      const p = e.point;
      if (
        ['park', 'zone', 'terrain', 'custom_building', 'water'].includes(state.tool) &&
        state.drawPoints.length >= 3
      ) {
        const firstP = state.map.project(state.drawPoints[0]);
        if (Math.hypot(p.x - firstP.x, p.y - firstP.y) < 20) {
        ...
        if (Math.hypot(p.x - lastP.x, p.y - lastP.y) < 20) {
          finishLine();
          return;
        }
      }
...
export function translateFeature(id, dlng, dlat) {
  const feats = state.features.filter(
    (x) => x.properties.id === id || x.properties.parent_id === id
  );
  feats.forEach((f) => {
    ...
    if (f.geometry.type === 'Point') f.geometry.coordinates = movePts([f.geometry.coordinates])[0];
      ...
      f.geometry.coordinates = movePts(f.geometry.coordinates);
    else if (f.geometry.type === 'Polygon')
      f.geometry.coordinates = f.geometry.coordinates.map(movePts);
  });
}
